$(document).ready(function () {
  ...
  $('.locations .popover > ul > li > input[type="checkbox"]').change(function () {
    if ($(this).is(':checked')) {
      ...
      stSelected[$(this).attr('data-name')] = $(this).attr('data-name');
    } else {
      stateId = stateId.filter(val => val !== $(this).attr('data-id'));
      delete stSelected[$(this).attr('data-name')];
    }
    $('.locations h4').text(Object.values(stSelected).join(', '));
  });
  ...
  $('.locations .popover > ul > li > ul > li > input[type="checkbox"]').change(function () {
    if ($(this).is(':checked')) {
      ...
      ctySelected[$(this).attr('data-name')] = $(this).attr('data-name');
    } else {
      cityId = cityId.filter(val => val !== $(this).attr('data-id'));
      delete ctySelected[$(this).attr('data-name')];
    }
    console.log(ctySelected);
    $('.locations h4').text(Object.values(ctySelected).join(', '));
  });
